...
export async function generateMetadata({ params }: Args): Promise<Metadata> {
  ...
  const description = service.seo?.description || `Affordable ${service.title.toLowerCase()} starting at $${service.startingPrice || 35} at Pet Care in Tucson. 40% less than the average vet. Walk-ins welcome, no hidden fees.`
...
export default async function ServicePage({ params }: Args) {
  ...
  const jsonLd = {
    ...
    priceRange: service.startingPrice ? `From $${service.startingPrice}` : '$',
    provider: {
      '@type': 'VeterinaryCare',
